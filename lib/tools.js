...
/** 注册全部 browser_* 工具 */
export function applyBrowserTools(register, deps) {
    ...
    const siteGate = async (exec) => {
        const r = await gates.ensureCurrentSiteAccess(exec.agent, exec.signal);
        if (r === "blocked")
            throw new Error("该网站已被阻止访问");
        if (r === "cancelled")
            throw new Error("未获得网站访问授权");
    };
    const fileUploadGuard = (type) => {
        if (type === "file") {
            throw new Error("网页要求上传文件，但内置浏览器不能自动完成文件上传（spec #8）。请用户手动上传，或改用其他工作流。");
        }
    };
    const activeTabId = async (exec) => {
    ...
    register(buildTool({
        ...
        description: "返回内置浏览器共享视图的链接（用户可直接打开查看/操作同一页面；页面与 Agent 所见一致）。",
        parameters: {},
        execute: async () => ({
    ...
    register(buildTool({
        ...
        description: "在内置浏览器中打开 URL（本地开发地址、file 页面或公开网页）。首次访问新网站会请求用户授权（spec #6）；敏感页面会请求确认（spec #7）。打开后用户可在共享视图实时看到。",
        parameters: {
        ...
        execute: async (args, exec) => {
            ...
            if (host) {
                ...
                if (r === "blocked")
                    throw new Error(`该网站已被阻止访问: ${host}`);
                if (r === "cancelled")
                    throw new Error("未获得网站访问授权");
            }
    ...
    register(buildTool({
        name: "browser_click",
        description: "点击当前页面中的元素（按文字 / label / name / href 定位）。提交、购买、删除等敏感动作会请求用户确认（spec #7）。文件上传控件无法自动点击（spec #8）。",
        parameters: {
        ...
        execute: async (args, exec) => {
            ...
            const risk = await gates.guardRisk({ kind: "click", elementText: el.text || el.name, element: { tag: el.tag, type: el.type, href: el.href } }, host, tab?.url ?? "", exec.agent, exec.signal);
            if (!risk.ok)
                throw new Error(risk.reason);
    ...
    register(buildTool({
        name: "browser_type",
        description: "在页面输入框中输入文本（按 placeholder / label / name / id 定位；query 为 * 时聚焦第一个输入框）。文件上传控件无法自动填写（spec #8）。",
        parameters: {
    ...
    register(buildTool({
        ...
        description: "读取当前页面的渲染状态：URL、标题、正文文本与可交互元素列表（含坐标）。每个操作后都应观察验证（spec #5）。",
        parameters: {},
    ...
    register(buildTool({
        ...
        execute: async (args, exec) => {
            ...
            if (url) {
                ...
                const r = await gates.ensureSiteAccess(host, url, "navigation", exec.agent, exec.signal);
                if (r !== "ok")
                    throw new Error(r === "blocked" ? "该网站已被阻止访问" : "未获得网站访问授权");
            }
            manager.setActorSession(exec.agent?.id ?? "");
    ...
    register(buildTool({
        ...
        description: "读取用户在页面上留下的标注（comments）。用户说「处理我在页面上的评论」时使用（spec #10）。",
        parameters: {
    ...
    register(buildTool({
        ...
        description: "读取当前页面的 console 输出（需要 Developer Mode 开启且该网站已批准 full CDP，spec #12）。",
        parameters: {},
        execute: async (_args, exec) => {
            await cdpGate(exec);
    ...
    register(buildTool({
        ...
        description: "读取当前页面的网络请求（需要 Developer Mode 开启且该网站已批准 full CDP，spec #12）。",
        parameters: {},
        execute: async (_args, exec) => {
            await cdpGate(exec);
