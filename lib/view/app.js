...
function applyExternalMode() {
  ...
  if (!local) {
    ...
    hint.innerHTML = '<div class="big">🌐</div><div><b>外部模式已启用（原生实时视图）</b></div><div style="font-size:12.5px;color:var(--text-3);line-height:1.8">当前已连接带调试端口的浏览器，实时 iframe 显示在驱动它的那个窗口里。<br />请查看已开启调试端口的浏览器窗口；本窗口可忽略或点「收起」。</div>';
...
$("#btn-frame")?.addEventListener("click", () => {
  ...
  // 防护：不能把共享视图/GUI 自身嵌进 iframe（会无限递归套娃）
  if (!state.iframeMode && (targetUrl.includes(":9333") || targetUrl.includes(":3080"))) {
    toast("该页面是共享视图/GUI 自身，不能以 iframe 嵌入（会递归），请先在地址栏打开目标网址", "warn");
    return;
  }
