...
const state = {
  ...
  iframeMode: false,
  external: false,
  relaunchDismissed: false,
  shotTs: 0,
};

// ---- 外部浏览器模式（路线 C）----
// 面板 #live-frame 就是 Agent 正在驱动的真实页面（原生渲染、实时同步）。
// 宿主会给「已连接的外部浏览器窗口」里的共享视图帧打 __dshExternal 标记：
// 只有被标记的本窗口才显示外部模式原生视图；其它窗口显示提示横幅。
function applyExternalMode() {
  if (!state.external) return;
  const local = typeof window.__dshExternal === "boolean" ? window.__dshExternal : false;
  if (!local) {
    // 本窗口不是被驱动的那一个（自动拉起的新窗口才是）
    let hint = document.getElementById("external-hint");
    if (!hint) {
      hint = document.createElement("div");
      hint.id = "external-hint";
      $("#stage-wrap").appendChild(hint);
    }
    hint.innerHTML = '<div class="big">🌐</div><div><b>原生实时视图已自动打开</b></div><div style="font-size:12.5px;color:var(--text-3);line-height:1.8">系统已为「内置浏览器」自动拉起一个新浏览器窗口（独立 profile，不影响你平时的浏览器）。<br />请切换到那个新窗口 —— 页面在那里原生渲染、实时同步，Agent 操作与你的操作完全一致。<br />本窗口可以忽略或点「收起」。</div>';
...
// ---- 一键重启浏览器（启用外部原生模式：实时 iframe）----
// 未进外部模式时展示卡片：允许后宿主重启当前浏览器（保留 profile、恢复窗口）并带调试端口，
// 之后共享面板里就是 Agent 驱动的实时 iframe（原生渲染、和 Trae 一样）。
let relaunching = false;
function showRelaunchCard() {
  if (relaunching) return;
  let card = document.getElementById("relaunch-card");
  if (card) return;
  card = document.createElement("div");
  card.id = "relaunch-card";
  card.innerHTML =
    '<div class="rc-icon">⚡</div>' +
    '<div class="rc-body">' +
    '<div class="rc-title">启用「原生实时视图」</div>' +
    '<div class="rc-desc">页面将在内置面板中<b>原生渲染、实时同步</b>（和 Trae/Codex 一样锐利）。' +
    '只需一次：允许后插件会重启你的浏览器（自动恢复所有窗口）并开启调试端口。' +
    '以后点 🌐 即用，无需再操作。</div>' +
    "</div>" +
    '<div class="rc-actions">' +
    '<button class="rc-btn" id="rc-go">重启浏览器并启用</button>' +
    '<button class="rc-x" id="rc-no" title="保持现在的截图流模式">暂不</button>' +
    "</div>";
  const wrap = $("#stage-wrap");
  wrap.appendChild(card);
  $("#rc-go").addEventListener("click", async () => {
    relaunching = true;
    card.innerHTML = '<div class="rc-icon">⏳</div><div class="rc-body"><div class="rc-title">正在重启浏览器…</div><div class="rc-desc">窗口会短暂关闭后自动恢复，然后在这里出现实时视图。</div></div>';
    try {
      const res = await api.post("/api/relaunch", { ua: navigator.userAgent });
      if (!res || !res.ok) {
        card.remove();
        toast("重启未成功：" + (res && res.error ? res.error : "未知原因"), "warn");
        relaunching = false;
        return;
      }
      // 成功：浏览器即将被重启，本页会消失。若 5s 后页面还在（浏览器没被重启），提示手动操作
      setTimeout(() => {
        if (document.getElementById("relaunch-card")) {
          card.remove();
          toast("浏览器似乎没有重启。请手动关闭浏览器后用 --remote-debugging-port=9222 启动，再点「🌐 浏览器」。", "warn");
          relaunching = false;
        }
      }, 5000);
    } catch (e) {
      card.remove();
      toast("重启请求失败：" + (e && e.message ? e.message : e), "warn");
      relaunching = false;
    }
  });
  $("#rc-no").addEventListener("click", () => {
    state.relaunchDismissed = true;
    card.remove();
    toast("已保持「截图流」模式（Agent 操作实时同步；高分屏清晰度一般）", undefined);
  });
}
...
async function refreshState() {
  ...
  if (data.external !== state.external) {
    ...
    // 标记/目标帧可能稍后才就绪（自动拉起的新窗口正在打开），重算一次
    applyExternalMode();
  }
  if (data.externalError && !data.external) {
    // 外部连接失败（新窗口还在启动）：每 2s 重试，直到连上
    let badge = document.getElementById("external-badge");
    if (!badge) {
      badge = document.createElement("div");
      badge.id = "external-badge";
    ...
    badge.textContent = "正在启动外部浏览器… " + data.externalError;
    if (!externalRetryTimer) {
      externalRetryTimer = setInterval(() => {
        externalRetryTimer = null;
        void refreshState().catch(() => {});
      }, 2000);
    }
  } else if (externalRetryTimer) {
    clearInterval(externalRetryTimer);
    externalRetryTimer = null;
  }
  // 一键重启浏览器卡片：仅当未进外部模式、浏览器还没启动、且本机有可重启的浏览器
  if (data.relaunchable && !state.external && !state.relaunchDismissed) {
    showRelaunchCard();
  } else {
    const card = document.getElementById("relaunch-card");
    if (card) card.remove();
  }
  renderTabs();
  renderAddress();
