...
export class ViewServer {
    ...
    async start() {
        ...
        if (!this.server)
            throw new Error("无法绑定共享视图端口（9333 起 20 个端口均被占用）");
    ...
    async handle(req, res) {
        ...
        const allowOrigin = origin === "http://127.0.0.1:3080" || origin === "http://localhost:3080" ? origin : undefined;
        if (allowOrigin) {
        ...
        try {
            ...
            // ---- 静态资源 ----
            if (isGet && (p === "/" || p === "/index.html"))
                return this.serveFile(res, path.join(this.opts.viewDir, "index.html"));
            if (isGet && p === "/styles.css")
                return this.serveFile(res, path.join(this.opts.viewDir, "styles.css"));
            if (isGet && p === "/app.js")
                return this.serveFile(res, path.join(this.opts.viewDir, "app.js"));
            if (isGet && p.startsWith("/shots/"))
                return this.serveFile(res, path.join(this.opts.shotsDir, path.basename(p)));
            if (isGet && p.startsWith("/demo")) {
                const rel = p === "/demo" || p === "/demo/" || p === "/demo/index.html"
                    ? "index.html"
                    : p.slice("/demo/".length);
            ...
            if (method === "GET" && p === "/api/events") {
                ...
                if (session)
                    this.sseSessions.set(res, session);
                req.on("close", () => {
            ...
            if (method === "GET" && p === "/api/screencast") {
                ...
                req.on("close", () => {
                    ...
                    if (this.streamClients.size === 0)
                        void manager.stopScreencast().catch(() => { });
                });
                return;
            }
            // ---- API ----
            if (p === "/api/state" && method === "GET") {
                ...
                try {
                    ...
                    if (manager.external)
                        await manager.refreshExternalTarget().catch(() => { });
                }
                catch (err) {
                ...
                return this.sendJson(res, 200, {
                    ...
                    relaunchable: !manager.external && !manager.connected && manager.canRelaunchBrowser,
            ...
            if (p === "/api/relaunch" && method === "POST") {
                if (manager.connected)
                    return this.sendJson(res, 400, { ok: false, error: "外部浏览器已连接，无需重启" });
                const body = await this.readBody(req);
                const ok = await manager.relaunchBrowserForExternal(String(body.ua ?? ""));
                if (!ok)
                    return this.sendJson(res, 500, { ok: false, error: "重启失败：找不到浏览器或调试端口未就绪" });
            ...
            if (p === "/api/input" && method === "POST") {
                ...
                if (!tab)
                    return this.sendJson(res, 400, { ok: false, error: "没有打开的标签页" });
            ...
            if (p === "/api/resize" && method === "POST") {
                ...
                if (!(w >= 320 && h >= 240))
                    return this.sendJson(res, 400, { ok: false, error: "尺寸过小" });
            ...
            if (p === "/api/annotation" && method === "POST") {
                ...
                if (!url)
                    return this.sendJson(res, 400, { ok: false, error: "无法确定标注页面" });
                ...
                if (tab)
                    manager.setAnnotationMode(tab.id, true).catch(() => { });
...
//# sourceMappingURL=view-server.js.map