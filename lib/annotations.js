...
export function parseAnnotationPayload(message, urlOfPage) {
    if (!message.startsWith(ANN_PREFIX))
        return null;
    try {
...
export function buildOverlayScript(annotations) {
    const serialized = JSON.stringify(annotations.map((a) => ({ id: a.id, x: a.x, y: a.y, w: a.w, h: a.h, comment: a.comment })));
    return OVERLAY_SCRIPT.replace("__CB_ANNOTATIONS__", serialized);
}
export function buildOverlayExitScript() {
