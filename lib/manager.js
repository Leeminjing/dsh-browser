...
export class BrowserManager {
    ...
    async launchExternalBrowser() {
        try {
            ...
            const p = spawn(exe, ["--remote-debugging-port=9222", `--user-data-dir=${profile}`, "--no-first-run", "--no-default-browser-check", `${gui}?dsh-browser=open`], { detached: true, stdio: "ignore" });
            p.unref();
            return true;
        }
        catch {
