...
export class BrowserManager {
    ...
    async adoptPage(page) {
        ...
        const syncNavState = async () => {
            ...
            try {
                tab.canGoBackCache = (await page.evaluate(() => window.history.length > 1));
            }
            catch {
    ...
    reload(tabId) {
        const t = this.tabs.get(tabId) ?? this.active();
        if (t)
            void t.page.reload({ waitUntil: "domcontentloaded" });
    }
    // ---- 页面操作 ----
    async findElement(tabId, query) {
        ...
        return (await t.page.evaluate(FIND_ELEMENT_SCRIPT(query)));
    }
    ...
    async type(tabId, query, text) {
        ...
        if (query !== "*") {
            ...
            await t.page.evaluate(`(() => { const el = document.querySelector('input,textarea,[contenteditable="true"]'); if (el) el.focus(); return !!el; })()`);
        }
    ...
    async snapshot(tabId) {
        ...
        if (!t)
            throw new Error("没有打开的浏览器标签页");
        const data = (await t.page.evaluate(SNAPSHOT_SCRIPT));
