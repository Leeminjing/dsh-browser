...
默认共享视图是隔离内置浏览器的 JPEG 截图帧流（任意站点可用，略有压缩/缩放损耗）。想要**像素级原生渲染 + Agent 实时控制**——和 Trae/Codex 的内嵌浏览器一模一样——**只需点一下「🌐 浏览器」按钮**：插件会自动拉起一个独立 profile 的浏览器窗口（调试端口 9222）、通过 CDP 连上，Agent 直接驱动那个窗口共享视图里的实时 iframe。

**一步到位，别的什么都不用做。** 老窗口会显示提示横幅，指向新弹出的窗口；若调试端口浏览器已在运行则直接复用。

- 自动探测端口：9222/9223/9224；可设置 `$env:DSH_BROWSER_NO_AUTO_LAUNCH = "1"` 关闭自动拉起。
- 手动方式：双击 `start-external.cmd`（或运行 `start-external.ps1`）效果相同；可选 `$env:DSH_BROWSER_CDP_URL` 强制指定端点。

> ⚠️ 外部模式**无隔离**（spec #3）：Agent 操作的是你的真实浏览器与真实登录态。仅在调试端口窗口运行时生效，徽标常驻提醒；标签页/后退前进 UI 已隐藏——Agent 只驱动面板内这一个目标帧。
