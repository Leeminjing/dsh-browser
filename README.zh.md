...
**默认**共享视图就是内置隔离浏览器，显示在 **GUI 面板内部**（JPEG 帧流，任意站点可用，自适应视口 + quality 85，1:1 清晰显示）。点「🌐 浏览器」即可，**不弹窗、无任何设置**。

想要**像素级原生渲染**（和 Trae/Codex 的内嵌浏览器一样）时，有可选的**外部模式**：Agent 通过 CDP 直接驱动共享面板里的实时 iframe——前提是你的浏览器带调试端口。用 `--remote-debugging-port=9222` 启动**你自己的浏览器**（如 `chrome.exe --remote-debugging-port=9222`）并在其中打开 DSH GUI；插件自动探测端口（9222/9223/9224），或用 `$env:DSH_BROWSER_CDP_URL` 指定。激活时面板显示原生 iframe 并带徽标 **外部浏览器模式 · 直连你的浏览器（无隔离）**；其它窗口显示提示横幅。

> ⚠️ 外部模式**无隔离**（spec #3）：Agent 操作的是你的真实浏览器与真实登录态。仅在调试端口浏览器运行时生效；标签页/后退前进 UI 已隐藏——Agent 只驱动面板内这一个目标帧。
