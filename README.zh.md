...
> 插件启用/停用由 Harness 的插件体系管理（spec #1）——未启用时 `browser_*` 工具不注册，
> Agent 不会执行任何内置浏览器操作。
...
│  ├─ gates.ts          门控：站点权限/高风险确认/历史授权/CDP 批准
│  │                    （走 GUI 用户问答——绑定调用会话，web 部署下可正常弹出）
...
│  ├─ demo-site/        Caspian 风格演示站点（/demo）+ watch.html（隐藏监听页：订阅 SSE，
│  │                    Agent 导航时 postMessage 通知 GUI 自动弹出共享视图面板）
│  └─ client/client.js  客户端插件：会话头部「🌐 浏览器」按钮 + 右侧停靠面板（iframe；分隔条可拖动，
│  │                    对话区与共享视图联动伸缩；Agent 操作时面板自动弹出，且按会话隔离）
├─ tests/               vitest 单元测试
└─ lib/                 构建产物（tsc + 静态资源复制）
```
...
| # | Spec 行为 | 实现 |
|---|-----------|------|
| 1 | 启用行为：未安装不执行；@Browser/明确任务可调用 | 插件由 Harness 插件体系启用/停用（cordis 行）；未启用则 `browser_*` 工具不注册 |
...
| 8 | 文件上传不能自动完成 | `type=file` 拦截；`filechooser` 事件仅提示；视图页提供「用户手动上传」 |
...
| 11 | 浏览历史：独立保存/搜索/删除；Agent 读取需授权 | `stores.recordHistory`（导航时）+ 视图页历史弹窗 + `browser_read_history`（授权后） |
...
1. 重启 Harness 后，对话输入 **`启动 Caspian 并在内置浏览器打开`**（或 `@Browser 打开 http://127.0.0.1:9333/demo/`）
   → 首次访问新站点弹出网站授权（spec #6）→ 允许 → Agent 打开页面并报告 `HTTP 验证返回 200`。
2. 点右上角「🌐 浏览器」→ 右侧停靠共享视图，能看到 Agent 正在操作同一页面（实时刷新）。
...
默认共享视图是隔离内置浏览器的 JPEG 截图帧流（任意站点可用，略有压缩/缩放损耗）。想要**像素级原生渲染 + Agent 实时控制**——和 Trae/Codex 的内嵌浏览器一模一样——开启**外部模式**：Agent 通过 CDP 直接驱动共享面板里的实时 iframe（就是你浏览器里的页面）。

**一键开启（只需 2 步）：**

1. **双击 [`start-external.cmd`](./start-external.cmd)**（或运行 `.\start-external.ps1`）→ 自动用独立 profile 启动带调试端口（9222）的 Chrome/Edge 并打开 DSH GUI（不影响你平时用的浏览器）。
2. **重启 harness**（`dsh web`）并刷新 GUI → 面板出现橙色徽标 **外部浏览器模式 · 直连你的浏览器（无隔离）**。

插件会自动探测调试端口（9222/9223/9224），**无需任何环境变量**。（可选：`$env:DSH_BROWSER_CDP_URL = "http://127.0.0.1:9222"` 可强制指定。）

> ⚠️ 外部模式**无隔离**（spec #3）：Agent 操作的是你的真实浏览器与真实登录态。仅在调试端口浏览器运行时生效，徽标常驻提醒；标签页/后退前进 UI 已隐藏——Agent 只驱动面板内这一个目标帧。

## 7. 已知边界（POC）

- 默认共享视图为「截图 + 点击叠加」（任意站点可用）；允许嵌入的站点可切 iframe 模式；外部模式画质原生，代价是无隔离。
...
- 内置浏览器本身是全局单实例；**面板 UI 与自动弹出按会话隔离**（只在操作会话自动弹出；切换会话面板自动收起）。
