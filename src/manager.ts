...
export class BrowserManager {
   ...
   */
  async relaunchBrowserForExternal(uaHint?: string): Promise<{ ok: boolean; error?: string }> {
    if (this.external) return { ok: false, error: "已处于外部模式，无需重启" };
    try {
      const { spawn, execSync } = await import("node:child_process");
      const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));
      const isRunning = (name: string): boolean => {
        try {
          const out = execSync(`tasklist /FI "IMAGENAME eq ${name}" /NH`, { encoding: "utf8", windowsHide: true, stdio: ["ignore", "pipe", "ignore"] });
      ...
      // 1) 确定要重启的浏览器：用户正在运行的优先（Edge 常用），其次 UA 提示，其次已安装的
      ...
      const edgeRunning = isRunning("msedge.exe");
      const chromeRunning = isRunning("chrome.exe");
      let exe = "";
      let name = "";
      if (edgeRunning && edgeCandidates.length) {
        exe = edgeCandidates[0];
        name = "msedge.exe";
      } else if (chromeRunning && chromeCandidates.length) {
        exe = chromeCandidates[0];
        name = "chrome.exe";
      } else if (/Edg\//i.test(uaHint ?? "") && edgeCandidates.length) {
        exe = edgeCandidates[0];
        name = "msedge.exe";
      } else if (chromeCandidates.length) {
        exe = chromeCandidates[0];
        name = "chrome.exe";
      } else if (edgeCandidates.length) {
        exe = edgeCandidates[0];
        name = "msedge.exe";
      } else {
        return { ok: false, error: "找不到 Chrome/Edge 可执行文件" };
      }

      // 2) 先释放内置隔离浏览器（若有），否则 ensure() 会跳过外部连接分支
      ...
      // 3) 关闭用户浏览器全部实例（profile 保留；--restore-last-session 会恢复窗口）
      try {
      ...
      // 4) 带调试端口重启：默认 profile + 恢复窗口 + 打开 GUI（自动展开面板）
      ...
      return { ok: false, error: "浏览器已重启但调试端口（9222）未就绪；可手动关闭后重新启动浏览器再试" };
    } catch (e) {
