...
export class BrowserStores {
  ...
  private load(): Persisted {
    try {
      if (fs.existsSync(this.file)) {
        const raw = JSON.parse(fs.readFileSync(this.file, "utf-8")) as Partial<Persisted>;
        const d = defaults();
        return {
  ...
  addAnnotation(a: Omit<Annotation, "id" | "ts">): Annotation {
    const ann: Annotation = { ...a, id: this.uid("a"), ts: Date.now() };
    this.data.annotations.push(ann);
    if (this.data.annotations.length > 500) this.data.annotations.splice(0, this.data.annotations.length - 500);
    this.save();
    return ann;
  }
...
export interface RiskInput {
  ...
  url?: string;
  elementText?: string;
  element?: { tag: string; type?: string; href?: string; download?: boolean };
  formMethod?: string;
}
...
export function assessRisk(a: RiskInput): { level: RiskLevel; description: string } {
  const text = norm(a.elementText);
  const url = norm(a.url);
  const type = norm(a.element?.type);

  for (const w of HIGH_WORDS) {
    if (text.includes(w)) return { level: "high", description: `「${a.elementText}」是不可撤销操作` };
  }
  if (a.kind === "navigate" && url) {
    for (const p of SENSITIVE_URL_PARTS) {
      if (url.includes(p)) return { level: "sensitive", description: `导航到敏感页面 ${a.url}` };
    }
  }
  ...
  if (a.kind === "submit" || (a.kind === "click" && (type === "submit" || norm(a.element?.tag) === "button"))) {
    if (a.formMethod === "post" || type === "submit") {
      for (const w of SENSITIVE_WORDS) {
        if (text.includes(w)) return { level: "sensitive", description: `「${a.elementText}」是敏感操作（提交类）` };
      }
    }
  }
  if (a.kind === "click" || a.kind === "type" || a.kind === "submit") {
    for (const w of SENSITIVE_WORDS) {
      if (text.includes(w)) return { level: "sensitive", description: `「${a.elementText}」是敏感操作` };
    }
  }
